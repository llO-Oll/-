...
对于B-tree,无论叶子节点还是非叶子节点，都保存数据，这样导致一页中存储的键值减少，指针也会减少，数据量大的情况下，只能增加树的高度，导致性能降低。
...
- **全局锁**：锁住数据库的所有表格。	备份数据库之前，需要加上全局锁。

- **表级锁**：

  1. 表锁：

     ①表共享读锁(read lock)：每个客户端都可以读，都不可以写。

     ②表独占写锁(write lock)：加锁客户端可以读/写，其它客户端不可以读/写。
  ...
  2. 元数据锁(meta data lock)：系统自动控制，保证读写的正确性。 比如，两个客户端同时对数据库操作，其中有一个客户端有未交交的事务，此时数据库就会自动加上元数据锁，阻止另外一个数据库的操作。

  3. 意向锁：使得表锁不用检查每行数据是否加锁，减少了表锁的检查。

     ①意向共享锁：`select ... lock in share mode`	与表锁共享锁(read)兼容，与表独占锁(write)互斥。

     ②意向排它锁：`insert、update、delete、 select ... for update`  与表锁共享锁(read)、表独占锁(write)都互斥。

- **行级锁**：对行数据加锁。

  1. 行锁：对单个行数据加锁，防止其它事务对此行进行update和delete。
  2. 间隙锁：锁定索引记录间隙，防止其它事务在这个间隙进行insert,产生幻读。
